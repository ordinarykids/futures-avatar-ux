...
export interface AiAvatarPanelProps {
  ...
  captionsText?: string;
  /** Audio URL to play through avatar */
  audioUrl?: string;
  className?: string;
}

export interface AiAvatarPanelHandle {
  playAudio: () => void;
  playAudioBlob: (arrayBuffer: ArrayBuffer) => Promise<void>;
}
 ...
 */
const AiAvatarPanel = forwardRef<AiAvatarPanelHandle, AiAvatarPanelProps>(
  (
    {
    ...
    const playAudio = useCallback(() => {
      if (audioUrl && interactiveAvatarRef.current) {
        // Check if the audioUrl is a blob URL or a file path
        if (audioUrl.startsWith("blob:")) {
          // For blob URLs, we need to fetch the audio data and convert it to a File or Blob
          fetch(audioUrl)
            .then((response) => response.blob())
            .then((blob) => {
              // Create a URL from the blob and pass it to the handleSendTestAudio
              const tempUrl = URL.createObjectURL(blob);
              interactiveAvatarRef.current?.handleSendTestAudio(tempUrl);
            })
            .catch((error) => {
          ...
          interactiveAvatarRef.current.handleSendTestAudio(audioUrl);
        }
      }
    }, [audioUrl]);

    const playAudioBlob = useCallback(async (arrayBuffer: ArrayBuffer) => {
      if (interactiveAvatarRef.current) {
        try {
          // Pass the ArrayBuffer directly to handleSendTestAudio
          await interactiveAvatarRef.current.handleSendTestAudio(arrayBuffer);
        } catch (error) {
          console.error("Error playing audio blob through avatar:", error);
        }
      }
    }, []);
...
AiAvatarPanel.displayName = "AiAvatarPanel";
