...
export default function TranscriptReader() {
  ...
  const avatarPanelRef = useRef<any>(null);
  ...
  async function handleGenerateTranscriptSpeech() {
    ...
    try {
      ...
      // bytesPerSecond = 24000 * 1 channel * 2 bytes = 48000
      const bytesPerPacket = 960; // 20 ms
      const pcmBytes = await audioBlob.arrayBuffer();
      ...
      for (let i = 0; i < pcmBytes.length; i += bytesPerPacket) {
        const appendMsg = {
          type: "append",
          data: pcmBytes.slice(i, i + bytesPerPacket),
        };
        ws.send(JSON.stringify(appendMsg));
        await new Promise((r) => setTimeout(r, 20)); // pace in real-time
      }
    } catch (error) {
  ...
  const playAudioThroughAvatar = async () => {
    try {
      if (!audioBlob) {
      ...
      if (avatarPanelRef.current) {
        ...
        reader.onload = async (e) => {
          ...
          if (arrayBuffer) {
            await avatarPanelRef.current.playAudioBlob(arrayBuffer);
          }
        };
      ...
      <main className="flex flex-col gap-[32px] row-start-2 items-center w-full max-w-xl">
        ...
        {audioUrl && (
          <div className="w-full">
            ...
            <button
              className="mt-4 px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 w-full"
              onClick={playAudioThroughAvatar}
            >
